...
**You do not need this folder.** The app hands you the same file: **Home → GET A
TEMPLATE** downloads `template.cartographer.json`, filled in and ready to edit.

- **`minimal.cartographer.json`** — the smallest file that works: a name and some
  titles. Everything else is filled in on import.
- **`project.cartographer.json`** — the full example, and the same file the GET A
  TEMPLATE button produces. Generated by `node --import tsx
  scripts/write-template.mjs` from `src/lib/template.ts`, so it cannot drift past
  what the importer accepts; `src/lib/template.test.ts` imports it on every run.
...
| `_readme` | Ignored on import. The downloaded template uses it to explain itself; delete it or leave it. |
...
| `cols` | `0` (auto) |
| `tags` | `[]` — cross-cutting labels, e.g. `["Frontier", "Merchant Guild"]`. A page sits on one board, so tags are the only way to group across boards. Matched case-insensitively, so "Guild" and "guild" are one tag. |
| `id` | generated |
...
`kind` is one of `text`, `number`, `long`, `ref`, `heading`, `date`, `select`.

A `select` carries its choices on the field:

```json
{ "key": "threat", "label": "Threat", "kind": "select",
  "options": ["Calm", "Tense", "Hostile"] }
```

A `ref` field's **value is the target page's `id`**, not its title — `"emberhold"`,
not `"Emberhold"`. An unambiguous title is accepted and resolved on import, but the
id is what is stored and what an export will contain.
